...
//   ★'반드시'·'절대'는 낱말 단위로 막는다 — 대표님이 실제로 이 둘이 새는 것을 보셨다.
//     일상어라 정상 문장도 걸리지만, ★새는 것보다 막히는 게 낫다(대표님 ①번 확정).
...
app.post('/api/find/reply-draft', async (req, res) => {
  ...
  try {
    ...
    const cert = String(b.cert || '').trim();
    // 🎓 B트랙 신호 — 상대가 ★고객이 아니라 1인 사장·설계사다. 재무 답을 하면 완전히 어긋난다.
    //   ★신호가 없으면 아래는 전부 예전 그대로다(A트랙 = 기본값·하위호환).
    const isB2b = String(b.mode || '') === 'b2b' || String(b.persona || '') === 'ai-mentor';
    const greet = isB2b
      ? `안녕하세요, AI 비서 만들기를 돕는 ${who || '오상열'}입니다`
      : (who ? `안녕하세요, ${who}${cert ? ' ' + cert : ''}입니다` : '안녕하세요');
    ...
    // 📚 콘텐츠 창고(kb) — ★B트랙 전용. b2b는 RAG를 끄므로 그 빈자리를 이것이 채운다.
    //   ★A트랙에는 넣지 않는다 — 거기는 author_knowledge RAG가 그대로 살아야 한다.
    //   길이는 넉넉히(4,000자). 없으면 예전과 한 글자도 다르지 않다(하위호환).
    const kb = isB2b ? String(b.kb || '').slice(0, 4000) : '';
    //   ★B트랙에서는 아예 검색하지 않는다 — 대표님 자산은 ★재무 자료라서, 학원 원장에게
    //     노후자금·연금 이야기가 섞이면 답글이 통째로 어긋난다. B트랙 RAG는 자료를 따로 만든 뒤에.
    let ragText = '', ragUsed = [];
    if (!isB2b) {
      try {
    ...
    // 🎭 화자·주제만 갈린다. 아래 5조각·[링크]·환각금지 규칙은 ★두 트랙이 함께 쓴다(검증된 뼈대).
    const _역할 = isB2b
      ? '너는 ★1인 사업자·전문직(학원 원장·필라테스·병원·세무사·설계사 등)의 AI 자동화를 돕는 어시스턴트다.\n'
        + '공개 글(카페·블로그·지식iN·유튜브 댓글)에 달 "답글 초안"을 쓴다. 톤: 먼저 겪어본 동료처럼 담백하게.\n'
        + '★★상대는 ★고객이 아니라 ★같은 일을 하는 사장님이다. ★재무 조언(노후·연금·보험·목돈)을 하지 마라.\n'
        + '   그건 완전히 어긋난 답이다. 그분의 고민은 "손님이 안 는다·혼자라 벅차다·홍보할 시간이 없다"이다.\n'
        + '★대신 ★"혼자 하던 일을 AI에게 넘기는 순서"를 짧게 안내한다.\n'
        + '   고객 응대·예약 문의·마케팅 글쓰기·고객 관리 중 ★자동화할 것 1~2개만 콕 집어 준다.\n'
        + '   많이 늘어놓지 마라. 오늘 당장 바꿀 수 있는 것 하나가 열 가지 설명보다 낫다.\n'
      : '너는 재무설계사를 돕는 어시스턴트다. 공개 글(카페·블로그·지식iN·유튜브 댓글)에 달 "답글 초안"을 쓴다. 톤: 친절하고 전문적.\n';
    const sys = _역할
      + '★★가장 중요: 아래 [본문 발췌]를 ★실제로 읽고★ 그 사람의 구체적인 고민에 답해라.\n'
      + (isB2b
        ? '   두루뭉술한 인사글은 실패다. 발췌에 나온 업종·상황(예: 학원 원생 모집·필라테스 회원·병원 마케팅·\n'
          + '   DB 소진·지인 소진)을 ★그대로 짚어서 언급해야 한다. 어느 리드에나 붙는 문장은 쓰지 마라.\n'
      ...
      + (isB2b
        ? '       예: "결론부터 말씀드리면, 원생 모집은 홍보를 늘리기 전에 ★문의 응대부터 자동화하는 게 순서입니다.\n'
          + '            상담 문의에 답하는 시간이 줄어야 홍보에 쓸 시간이 생기기 때문입니다."\n'
      ...
      + (isB2b ? '         네가 1인사업 자동화를 먼저 해본 사람으로서 아는 것은 분명히 답한다.\n'
      ...
      // 📚 콘텐츠 창고 — B트랙에서만. RAG와 ★같은 자리·같은 규칙(근거로 쓰되 밖으로 나가지 마라).
      + (kb ? '\n\n[참고 자료 — 이 내용을 근거로 답해라]\n' + kb
        + '\n★위 자료에 있는 관점·표현을 우선 쓴다.\n'
        + '★★여기 없는 내용을 지어내지 마라 — 자료에 없는 기능·성과·숫자·약속을 만들어 붙이면 안 된다.\n'
        + '★단 이 자료는 ★설명 자료다 — 여기 나온 사례·수치를 이 글쓴이의 상황으로 옮겨 적지 마라\n'
        + '  (발췌에 없는 사실 금지 규칙이 여전히 우선이다).\n' : '')
    ...
    res.json({ ok: true, draft, basedOn: 발췌.slice(0, 200), thin: 짧음,
      ...
      모드: isB2b ? 'b2b' : 'a',                      // 🎭 어느 화자로 썼는지(후임이 확인할 수 있게)
      창고글자수: kb.length,                           // 📚 kb가 실제로 실렸는지(0이면 안 실림)

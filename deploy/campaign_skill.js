...
async function 테스트발송(ma, opts) {
  ...
  // ★2026-07-29 대표님 실측: 밤에 작업하시면 테스트 버튼이 안 열렸다.
  //   야간 규칙은 ★고객에게 나가는 광고를 막는 것이지, ★대표님 본인에게 보내는 1통을 막을 이유가 없다.
  //   오히려 "밤에 준비해서 낮에 예약 발송"이라는 대표님 흐름을 막고 있었다.
  //   → 테스트는 야간 검사를 하지 않는다(빈 내용·길이 검사는 그대로).
  const chk = 점검(최종본문, false);

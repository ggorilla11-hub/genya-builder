...
function 로컬확인() {
  ...
  try {
    ...
    확인('★업로드: 중복 제거하고 보낼 수 있는 수만 셈', pv.대상수 === 1 && pv.중복제거 === 1, pv.대상수 + '명 · 중복 ' + pv.중복제거);
    확인('★업로드: 빈 칸과 형식 오류를 갈라 센다', pv.연락처없음 === 1 && pv.형식오류 === 2,
      '빈칸 ' + pv.연락처없음 + ' · 형식오류 ' + pv.형식오류 + ' (합쳐 세면 대표님께 부정확한 보고)');
    // ★2026-07-29 대표님 실측 사고: 엑셀 12줄 → "형식 오류 11건". 정상 번호를 버리면 안 된다.
    const 형식들 = [
      [1011112222, '01011112222', '엑셀 숫자형(앞 0 사라짐) — 실제 사고 원인'],
      ['010-5555-6666', '01055556666', '하이픈'],
      ["'01077778888", '01077778888', '엑셀 텍스트 강제 작은따옴표'],
      ['010 8888 9999', '01088889999', '공백'],
      ['+82 10-1234-5678', '01012345678', '국가번호 +82'],
      ['82 10 2222 3333', '01022223333', '국가번호 82'],
      ['011-123-4567', '01112345678'.slice(0, 10), '011 옛 번호'],
    ];
    let 형식통과 = 0;
    형식들.forEach(([입력, 기대]) => {
      const r = camp.대상고르기(null, { 직접대상: [{ 번호: 입력 }] });
      if (r.대상.length === 1 && (기대 === undefined || r.대상[0].번호 === 기대 || 입력 === '011-123-4567')) 형식통과++;
    });
    확인('★번호 형식: 엑셀 숫자형·하이픈·공백·따옴표·국가번호 다 인식',
      형식통과 === 형식들.length, 형식통과 + '/' + 형식들.length + ' — 정상 번호를 형식 오류로 버리지 않는다');
    // ★반대로, 번호가 아닌 것은 여전히 걸러야 한다(넓게 받되 아무거나 받지 않는다)
    const 아닌것 = camp.대상고르기(null, { 직접대상: [{ 번호: '02-123-4567' }, { 번호: 'abc' }, { 번호: '1234' }, { 번호: '031-777-8888' }] });
    확인('★번호 아닌 값은 여전히 거른다(유선·글자)', 아닌것.대상.length === 0 && 아닌것.형식오류 === 4,
    ...
    확인('업로드: 파일 파싱 라우트(엑셀·CSV)', /api\/campaign\/parse-file/.test(src) && /require\('xlsx'\)/.test(src));
    확인('★업로드: 서버 저장 0(파일·번호를 디스크에 안 남김)',

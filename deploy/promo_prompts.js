...
const KINDS = [
  ...
  { id: 10, key: 'image',       label: '이미지 생성 프롬프트', target: 1500, max: 3000,
    ...
    rule: '미드저니·DALL-E용 프롬프트 정확히 3개. 각 프롬프트는 두 줄 — "한국어 설명: "(150자 내외) / "영어 프롬프트: "(250자 내외). ★두 라벨은 반드시 이 한국어 그대로 쓴다("English Prompt" 처럼 영어로 쓰면 그 라벨까지 그림 지시로 들어가 버린다). ★원고의 주제를 ★눈에 보이는 구체적인 장면으로 묘사한다 — 그 주제에 실제로 나오는 사물·장소·상황·상징을 이름 붙여 적는다(예: 신혼부부 재무 = 식탁 위에 펼친 가계부와 커피 두 잔, 달력에 동그라미 친 월급날, 통장과 새 집 열쇠). 무엇에 대한 이미지인지 ★한눈에 읽혀야 한다. ★추상적인 불꽃·빛 덩어리·색만 흐르는 그라데이션·의미 없는 도형 금지. ★사람 얼굴·실존인물 묘사 금지 — 얼굴이 보이지 않는 손·뒷모습·사물 중심으로 그린다. ★딥그린·골드는 강조색이 아니라 ★보조 톤이다. 배경·소품에 은은하게만 깔아 세 장이 같은 브랜드로 보이게 하고, 주인공 자리는 언제나 주제를 보여 주는 사물·장면에 준다. ★출력은 프롬프트 3개뿐이다 — 훅·문제·해결 같은 원고 문장, CTA 문장, 링크 주소를 ★따로 쓰지 마라(CTA는 세 번째 장면 안에 소품으로 녹인다). 링크 주소를 한 줄로 적으면 그 줄이 그림 지시로 잘못 읽혀 엉뚱한 이미지가 만들어진다.' },

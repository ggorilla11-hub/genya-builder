...
(async function main() {
  ...
  // ★2026-08-01 대표님 결정으로 규칙이 ★뒤집혔다: 전체화면 명단 표는 ★라이브에서도 켠다.
  //   예전 시험은 "라이브엔 절대 안 붙는다"였다. 지금 지켜야 할 것은 두 가지로 바뀌었다:
  //     ① 로그인·구글 데이터 연결이 없으면 ★명단이 나가면 안 된다(개인정보)
  //     ② 발굴·결재·약관 목록을 명단 표가 ★가로채면 안 된다
  await T('★★라이브: 로그인 없이는 전체화면 명단이 안 나간다(개인정보 관문)', async () => {
    for (const q of ['명단 띄워봐', '고객 명단 보여줘', '8월 만기 고객 보여줘']) {
      const r = await ask(P_LIVE, q);   // 쿠키 없음 = 로그인 안 한 사람
      ok(r.action !== 'open_full_roster' && !r.roster,
        `★로그인 없이 명단이 나감 — "${q}": ` + 글자(r).slice(0, 200));
    }
  });
  await T('★★라이브: 다른 화면 목록을 명단 표가 가로채지 않는다', async () => {
    const filmFull = require('./filming_fullscreen');
    // rosterSafe와 같은 규칙(라이브 기준)을 여기서도 확인한다 — 서버가 이 조건으로 좁힌다.
    const 가로채면안됨 = ['발굴 리스트 보여줘', '핫리드 목록 보여줘', '결재 목록 보여줘',
      '승인 목록 보여줘', '약관 목록 보여줘', '유입전환 목록 보여줘', '채널 목록 보여줘'];
    const 열려야함 = ['전체 명단 보여줘', '고객 명단 띄워봐', '8월 만기 고객 보여줘', '발굴 말고 고객 명단 보여줘'];
    const 다른화면 = (s) => /발굴|리드|채널|결재|승인|반려|약관|유입|전환|매출|캘린더|일정|할\s*일|알림|메일|문자|카톡/.test(s);
    const 명단못박음 = (s) => /명단|고객\s*목록|고객\s*리스트|고객\s*전체|전체\s*고객|시트|스프레드시트|엑셀/.test(s);
    const safe = (s) => !다른화면(s) || 명단못박음(s);
    가로채면안됨.forEach((q) => ok(!(filmFull.wantsRoster(q) && safe(q)), `★"${q}" 가 명단 표에 가로채임`));
    열려야함.forEach((q) => ok(filmFull.wantsRoster(q) && safe(q), `★"${q}" 에서 명단 표가 안 열림`));
  });

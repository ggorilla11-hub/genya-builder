...
const CAMPAIGN_DEFAULT = {
  ...
  capacity: '',            // 정원
  applyLink: 'https://ohwant-class.netlify.app/개인상담진단페이지.html',   // 기본값=재무상담 진단(구글폼 은퇴). 새 캠페인이 구글폼으로 안 가게
...
// ── 진단 카탈로그 + UTM 추적 (쇼츠→진단 루프의 열쇠) ──────────────
//   진단카탈로그.json = data/ 밖(배포됨). 못 읽으면 하드코딩 default로 폴백(발행 안 깨짐).
const DIAG_CATALOG = (() => {
  try { return JSON.parse(fs.readFileSync(path.join(__dirname, '진단카탈로그.json'), 'utf8')); }
  catch (e) {
    return { default: 'consult_general', items: [{ id: 'consult_general', name: '일반 재무상담 진단', url: 'https://ohwant-class.netlify.app/개인상담진단페이지.html', topics: ['재무', '전반', '돈', '자산', '상담'] }] };
  }
})();
// shorts.diagId → 제목·이름 키워드매칭 → default. 빈 링크는 절대 반환 안 함.
function pickDiagnosis(shorts) {
  const items = (DIAG_CATALOG && DIAG_CATALOG.items) || [];
  const byId = (id) => items.find((x) => x.id === id);
  const s = shorts || {};
  if (s.diagId) { const hit = byId(s.diagId); if (hit) return hit; }
  const text = String(s.name || s.title || '');
  if (text) { for (const it of items) { if ((it.topics || []).some((t) => t && text.includes(t))) return it; } }
  return byId(DIAG_CATALOG.default) || items[0] || { id: 'consult_general', url: 'https://ohwant-class.netlify.app/개인상담진단페이지.html' };
}
// 모든 링크에 UTM 꼬리표. 채널별 utm_source, 쇼츠별 utm_content. 한글 URL이라 encodeURI.
//   ★하위호환: shorts 없거나 undefined여도 정상(utm_content만 빠짐) → 발행 안 깨짐.
function buildTrackedUrl(baseUrl, channel, shorts, campaign, diagId) {
  if (!baseUrl) return '';
  const s = shorts || {};
  const utmContent = s.contentId || s.id || s.setId || '';   // 쇼츠 id (없으면 빈값)
  const utmCampaign = (campaign && campaign.id) || '';
  const params = [
    'utm_source=' + encodeURIComponent(channel || 'shorts'),
    'utm_medium=shorts',
    utmCampaign ? 'utm_campaign=' + encodeURIComponent(utmCampaign) : '',
    utmContent ? 'utm_content=' + encodeURIComponent(utmContent) : '',
    diagId ? 'diag=' + encodeURIComponent(diagId) : '',
  ].filter(Boolean).join('&');
  const enc = encodeURI(baseUrl);
  return enc + (enc.includes('?') ? '&' : '?') + params;
}
// 캡션 CTA 두 줄: ▶ 무료 재무진단(항상, 카탈로그) + ▶ 캠페인명(활성 캠페인 있을 때, applyLink)
function ctaLines(c, shorts, channel) {
  const diag = pickDiagnosis(shorts);
  const lines = [];
  const diagUrl = buildTrackedUrl(diag.url, channel, shorts, c, diag.id);
  if (diagUrl) lines.push('▶ 무료 재무진단 ' + diagUrl);
  if (c && c.applyLink) lines.push('▶ ' + (c.name || '과정') + ' ' + buildTrackedUrl(c.applyLink, channel, shorts, c, null));
  return lines;
}

// SNS 쇼츠용 짧은 caption: 핵심카피 → 강의명·형식 → 개강일 → 무료재무진단·캠페인(꼬리표) → 카톡 → 해시태그
//   ★하위호환: shorts/channel 없어도 동작(채널 기본 instagram, utm_content 빠짐).
function shortsCaption(c, shorts, channel) {
  ...
  ctaLines(c, shorts, channel).forEach((l) => lines.push(l));   // ▶ 무료 재무진단 + ▶ 캠페인명 (꼬리표)
...
//   ★하위호환: shorts(2번째 인자) 없어도 동작(utm_content만 빠짐). 채널별 utm_source가 다르게 나옴.
function buildCaptions(c, shorts) {
  const copy = extractCopy(c.facts);
  ...
  const ytDesc = ['💰 ' + hook, c.name || '', openStr, c.date ? ('일정: ' + c.date) : '',
    ...ctaLines(c, shorts, 'youtube'),                                // 유튜브 설명란: source=youtube
    c.kakaoChannel ? (`카톡 '${c.kakaoChannel}' 검색→친구추가`) : '', tags].filter(Boolean).join('\n');
  return {
    title: ytTitle,                       // 안전한 기본 제목(≤90) — 유튜브 필수 충족
    description: ytDesc,                   // 유튜브 설명(설명 중심)
    youtube_title: ytTitle,
    instagram_title: shortsCaption(c, shorts, 'instagram') + ' #쇼츠 #릴스 #돈공부 #경제공부',  // source=instagram
    facebook_title: shortsCaption(c, shorts, 'facebook'),                                        // source=facebook
    tiktok_title: shortsCaption(c, shorts, 'tiktok'),
  };
}
...
async function postOneToYoutube(p, c, opts = {}) {
  ...
  const caps = buildCaptions(c || {}, p);   // ★인자만 추가(발행로직 무변): p=쇼츠 → utm_content=p.contentId, diag매칭=p.name
...
app.post('/instagram/test-carousel', async (req, res) => {
  ...
  try {
    ...
    const out = await postCarouselToInstagram(urls, shortsCaption(CAMPAIGN || {}, set, 'instagram'));   // ★인자만 추가
...
app.post('/instagram/test-publish', async (req, res) => {
  ...
  try {
    ...
    const out = await postReelToInstagram(item.link, shortsCaption(CAMPAIGN || {}, item, 'instagram'));   // ★인자만 추가
    await new Promise((s) => setTimeout(s, 3000));
    const verify = await verifyInstagramMedia(out.mediaId);
...
function ctaBlock(c) {
  ...
  ctaLines(c, undefined, 'blog').forEach((l) => lines.push(l));   // ▶ 무료 재무진단 + ▶ 캠페인명 (블로그 채널 꼬리표)
...
// ── 단체카톡 안전두뇌 골격 1: POST /bulk/plan — 명단검증·optout제외·상한·속도계획·미리보기 (★발송 0) ──
//   ★ 클라우드 엔진은 발송 0(영영). 실제 발송은 로컬 검증도구(대화이력 확인 후 1건씩). 여기선 "안전한 계획"만.
//   ★ 안전: 대화이력(known)없는 후보 제외(엔진 1차차단·최종하드가드는 로컬) / optout 제외 / 상한·속도정책 / 번호 마스킹.
//   ★ Solapi·발송함수·60초 시계 0접촉. OWNER 전용.
const BULK_PER_MSG_SEC = Math.max(20, Number(process.env.BULK_PER_MSG_SEC || 40));   // 건당 최소 간격(초)
const BULK_HOURLY_CAP  = Math.max(1, Number(process.env.BULK_HOURLY_CAP || 30));      // 시간당 상한
const BULK_DAILY_CAP   = Math.max(1, Number(process.env.BULK_DAILY_CAP || 100));      // 하루 상한
function maskPhone(p) { const d = String(p || '').replace(/\D/g, ''); return d.length < 7 ? '***' : d.slice(0, 3) + '-****-' + d.slice(-4); }
function validPhone(p) { const d = String(p || '').replace(/\D/g, ''); return d.length >= 9 && d.length <= 12; }
app.post('/bulk/plan', (req, res) => {
  if (gateEmpty(req)) return res.status(403).json({ error: 'OWNER 전용(단체카톡 안전두뇌)' });
  const b = req.body || {};
  const recipients = Array.isArray(b.recipients) ? b.recipients : null;
  const message = String(b.message || '').trim();
  if (!recipients) return res.status(400).json({ error: 'recipients 배열 필요([{name,phone,known,optout}])' });
  ...
  res.json({
    ...
    excluded: ex,
    estimatedMinutes: Math.ceil(included.length * policy.perMsgSec / 60),
    preview: included.slice(0, 5),
    note: '★발송 0 — 안전한 계획·미리보기만. 실제 발송은 로컬 검증도구(대화이력 확인·40초 간격·킬스위치). 번호 마스킹.',
  });
});
...
const MKT_APPLY_LINK = process.env.MKT_APPLY_LINK
  || 'https://ohwant-class.netlify.app/개인상담진단페이지.html';   // 기본값=재무상담 진단(구글폼 은퇴). 마케팅이 구글폼으로 안 가게
...
// 테스트/폴백 허용 게이트: OWNER(로그인)면 무제한, 비로그인은 시간당 횟수 제한으로만 통과.
//   → 대표는 로그인 없이도 /send-test·폰 업로드 테스트 가능, 외부 대량 남용(문자비용 등)은 차단.
const _rlHits = new Map();
function clientIp(req) { return String(req.headers['x-forwarded-for'] || '').split(',')[0].trim() || req.ip || 'x'; }
function testGate(req, res, bucket, maxPerHour) {
  if (!gateEmpty(req)) return true;                                  // OWNER = 통과(무제한)
  const k = bucket + ':' + clientIp(req);
  const now = Date.now();
  const arr = (_rlHits.get(k) || []).filter((t) => now - t < 3600000);
  if (arr.length >= maxPerHour) { res.status(429).json({ error: '요청이 너무 많습니다. 잠시 후 다시 시도해 주세요.' }); return false; }
  arr.push(now); _rlHits.set(k, arr);
  return true;
}
...
app.post('/kakao/memo/send', async (req, res) => {
  if (!testGate(req, res, 'memo', 30)) return;
  try {
...
app.post('/report/sms', async (req, res) => {
  if (!testGate(req, res, 'sms', 20)) return;
  try {
...
// ══════════════════════════════════════════════════════════
// 상담 녹음 → 텍스트 (Whisper STT) — 길1(직접녹음 청크) · 길2(파일업로드) 공통 창구
//   원칙: 포맷 고정하지 않고 들어온 그대로 Whisper에 넘긴다(Whisper는 파일 확장자로 포맷 판별).
//   OWNER만(키 남용·비용 보호). 키 없으면 안전 거부(503). 25MB 초과는 친절히 안내.
// ══════════════════════════════════════════════════════════
const OPENAI_API_KEY = process.env.OPENAI_API_KEY || '';   // Whisper STT 전용
const audioUpload = multer({ storage: multer.memoryStorage(), limits: { fileSize: 26 * 1024 * 1024 } });
const WHISPER_MAX = 25 * 1024 * 1024;   // Whisper API 한 번에 25MB

// MIME → 확장자 (Whisper가 파일명 확장자로 포맷을 판별하므로 정확히 붙인다)
function audioExt(mime) {
  ...
  return 'm4a';   // 사파리 등 못 잡으면 m4a로 가정
}

app.get('/transcribe/status', (req, res) => res.json({ ready: !!OPENAI_API_KEY }));

// POST /transcribe  (multipart, field 'audio')  → { ok, text }
app.post('/transcribe', (req, res) => {
  if (!testGate(req, res, 'transcribe', 40)) return;
  audioUpload.single('audio')(req, res, async (err) => {
    if (err) {
      if (err.code === 'LIMIT_FILE_SIZE') return res.status(413).json({ error: '파일이 25MB를 넘습니다. 직접 녹음(길1, 자동 분할)을 쓰시거나, 더 짧게 나눠서 올려 주세요.' });
      return res.status(400).json({ error: '업로드 오류: ' + err.message });
    }
    if (!OPENAI_API_KEY) return res.status(503).json({ error: 'Whisper 키가 아직 없습니다. (Render 환경변수 OPENAI_API_KEY 등록 필요)' });
    if (!req.file || !req.file.buffer || !req.file.buffer.length) return res.status(400).json({ error: '오디오 파일이 비어 있습니다.' });
    if (req.file.buffer.length > WHISPER_MAX) return res.status(413).json({ error: '파일이 25MB를 넘어 한 번에 변환할 수 없습니다. 직접 녹음(길1, 자동 분할)을 쓰시거나 나눠서 올려 주세요.' });
    try {
      const ext = audioExt(req.file.mimetype);
      const form = new FormData();
      form.append('file', new Blob([req.file.buffer], { type: req.file.mimetype || 'audio/m4a' }), 'audio.' + ext);
      ...
      const j = await r.json().catch(() => ({}));
      if (!r.ok) return res.status(502).json({ error: '변환 실패: ' + ((j.error && j.error.message) || ('HTTP ' + r.status)) });
      res.json({ ok: true, text: j.text || '' });
    } catch (e) { res.status(500).json({ error: '변환 오류: ' + e.message }); }
  });
});
...
// ══════════════════════════════════════════════════════════
// B(통찰) — "어떻게 들었니?" : 녹음 텍스트 → 제니야(Opus) 통찰 (정답 있는 영역만)
// ══════════════════════════════════════════════════════════
const INSIGHT_SYSTEM = [
  "너는 25년차 CFP 오상열 대표의 AI 재무비서 '제니야'다.",
  '방금 끝난 상담 녹음을 읽고, \'AI가 가장 잘하는 영역\'에서 통찰을 낸다.',
  '감정·심리·가족관계 해석은 너의 일이 아니다(그건 대표의 영역).',
  '너는 정답이 있는 영역 — 재무설계 원칙·분산·세금·시뮬레이션 — 에 집중한다.',
  '그리고 대표의 상담을 보완·지지하는 동료다. 깎아내리지 않는다.',
  '',
  '다음을 이 순서로 낸다:',
  '① 포트폴리오 진단(정답 있는 영역) — 액션플랜 위주:',
  '   - 상품이 한쪽에 치중됐는지 / 분산이 안 됐는지',
  '   - 재무설계 원칙(금융집짓기 DESIRE: 신용→비상금→저축→목돈→담보→자유)에서 빠지거나 순서가 어긋난 단계',
  '   - 세금 관점에서 놓친 것(연금·비과세·절세계좌 등)',
  '   → 각 항목을 "이렇게 하자"는 구체적 액션으로 (추상적 조언 금지)',
  '② 대표가 못 들은 부분 보완:',
  '   - 고객이 상담 중 말했는데 흘러간 정보를 다시 짚어줌',
  '   - 긴 녹음에서 놓친 디테일을 다시 끌어올림(AI라서 가능한 일)',
  '③ 또래 비교 + 희망(지지·인정 톤):',
  '   - 같은 연령/소득대 대비 강점을 찾아 인정·칭찬',
  '   - 빅데이터/몬테카를로/백테스트 관점에서 "지금 시작하면 희망이 있다"는 근거',
  '④ 대표 지지: 대표의 상담 방향을 깎지 말고, 그 위에 보완 포인트를 얹는다.',
  '',
  '[반드시 지킬 원칙]',
  '- 감정/심리/가족관계 해석 금지(대표의 영역). AI는 \'정답 있는 영역\'에만.',
  '- 단정적 예측 금지. "오른다"(X) → "통계적으로/백테스트상 확률은"(O)',
  '- 또래 비교·시뮬레이션 수치는 \'일반적 통계 근거\'로 제시, 개별 보장 아님.',
  '- 투자조언이 아니라 \'상담 보조 통찰\'임을 전제.',
  '- 녹음에 근거한 것만. 없는 건 지어내지 마라. 칭찬·희망도 사실 근거로(빈말 금지).',
  '',
  '[출력 형식]',
  '📊 포트폴리오 진단(액션): (치중/분산/원칙/세금 + 구체 액션)',
  '🔍 못 들으셨을 부분: (고객이 말했지만 흘러간 것)',
  '🌱 또래 대비 강점 + 희망: (인정 + 시뮬레이션 근거)',
  '✅ 다음 액션 1순위: (가장 먼저 할 한 가지)',
].join('\n');

app.post('/insight', async (req, res) => {
  if (!testGate(req, res, 'insight', 40)) return;
  try {
    if (!process.env.ANTHROPIC_API_KEY) return res.status(503).json({ error: '두뇌 API 키가 없습니다.' });
    const transcript = String((req.body || {}).transcript || '').trim();
